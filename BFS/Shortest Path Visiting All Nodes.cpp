//  https://leetcode.com/problems/shortest-path-visiting-all-nodes/description/?envType=study-plan&id=graph-i
...
public:
    int shortestPathLength(vector<vector<int>>& graph) {
      ...
      queue<pair<int, pair<int, int>>> q;  // Queue will store {node, {distance, mask}}
      set<pair<int, int>> vis;  // Visited set will store {node, mask}
      for(int i = 0; i < n; i++){
        int mask = 1 << i;
      ...
      while(!q.empty()){
        ...
        for(auto &node: graph[currNode]){
          ...
          if(vis.find({node, newMask}) != vis.end()) continue;
          else {
